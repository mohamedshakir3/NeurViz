...
class GAN():
    ...
    def parse_gen(self, layers): 
        ...
        if layers[0]["type"] == "Dense":
            res.append(nn.Linear(self.gan["hyperparameters"]["noiseDim"], layers[0]["out_channels"]))
    ...
    def parse_disc(self, layers): 
        res = []
        if layers[0]["type"] == "Dense":
            res.append(nn.Linear(self.gan["generator"][-1]["out_channels"], layers[0]["out_channels"]))
        elif layers[0]["type"] == "Conv2d": 
            res.append(nn.Conv2d(
                            self.gan["hyperparameters"]["noiseDim"], 
                            layers[0]["out_channels"], 
    ...
    def train(self, jobID):        
        ...
        supabase.table("jobs").update({ "status": "completed" }).eq("id", jobID).execute()
